...
<div style="background:linear-gradient(135deg,#f0f9ff,#fff);border-radius:8px;padding:16px;margin-bottom:16px;border:1px solid #bae6fd">
  ...
  if (cluster?.must_have?.length || cluster?.focus_plan?.length) {
    html += `
<div style="background:linear-gradient(135deg,#faf5ff,#fff);border-radius:8px;padding:16px;margin-bottom:16px;border:1px solid #e9d5ff">
  <h2 style="margin:0 0 12px;font-size:16px;color:#7e22ce">📊 JD 共性要求 · 专项学习</h2>
  <div style="font-size:12px;color:#666;margin-bottom:10px">基于 ${cluster.total_jobs || 0} 个相关岗位的技能覆盖率聚类</div>
  ${cluster.must_have?.length ? `
  <div style="margin-bottom:10px">
    <div style="font-size:12px;font-weight:600;color:#7e22ce;margin-bottom:6px">必备共性（覆盖率 ≥ 35%）</div>
  ...
  ${cluster.should_have?.length ? `
  <div style="margin-bottom:10px">
    <div style="font-size:12px;font-weight:600;color:#7e22ce;margin-bottom:6px">重要共性（20%–34%）</div>
    ${cluster.should_have.slice(0, 8).map(s => `
    <span style="display:inline-block;background:#faf5ff;color:#7e22ce;font-size:11px;padding:3px 8px;border-radius:4px;margin:0 4px 4px 0;border:1px solid #e9d5ff">${escapeHtml(s.skill)} ${s.coverage}%</span>`).join('')}
  </div>` : ''}
  ${cluster.focus_plan?.length ? `
  <div>
    <div style="font-size:12px;font-weight:600;color:#7e22ce;margin-bottom:6px">本周聚焦</div>
    ${cluster.focus_plan.slice(0, 6).map(p => `
    <div style="font-size:12px;color:#444;margin-bottom:6px;padding:8px;background:#fff;border-radius:6px;border:1px solid #f3e8ff">
      <span style="display:inline-block;background:${p.priority === 'P0' ? '#fee2e2' : p.priority === 'P1' ? '#ffedd5' : '#f1f5f9'};color:${p.priority === 'P0' ? '#991b1b' : p.priority === 'P1' ? '#9a3412' : '#64748b'};font-size:10px;padding:1px 6px;border-radius:3px;margin-right:6px">${escapeHtml(p.priority)}</span>
      <strong>${escapeHtml(p.skill)}</strong> — ${escapeHtml(p.action)}
    </div>`).join('')}
  </div>` : ''}
</div>`;
  }
